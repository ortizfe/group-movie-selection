...
function resolveDbPath() {
  ...
  return process.env.MOVIES_SQLITE
    ? path.resolve(process.env.MOVIES_SQLITE)
    : path.resolve(__dirname, '..', '..', 'data', 'processed', 'movies.sqlite');
}

/** Return a singleton DB connection */
...
/** Create tables if they don’t exist; keep schema aligned with ingest script */
export function migrate() {
  ...
  db.exec(`
    ...
    -- Presently empty (optional); safe for LEFT JOINs in recommendations route
