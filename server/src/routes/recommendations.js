...
r.post('/recommendations', (req, res) => {
  const db = getDb();
  const vibe = req.body || {};
  const providers = Array.isArray(vibe.providers) && vibe.providers.length
    ? vibe.providers
    : null;
  ...
  // Pull a reasonable candidate pool; scoring will sort it.
  ...
  const scored = rows
    .map(r => {
      ...
      return {
        ...r,
        score,
        matches: dbg.matches,  // per-axis 0..1 similarity
        axis: dbg.axis,        // movie axis values in [-1, +1]
        target: dbg.target     // slider targets in [-1, +1]
      };
    })
